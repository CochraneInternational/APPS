...
-(void)saveshit
{
    ...
    [self.frameView.layer renderInContext:context];
...
{
    [super viewDidLoad];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(saveshit) name:@"saveObject" object:nil];
    self.frameView =[[HFImageEditorFrameView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
    self.frameView.backgroundColor=[UIColor clearColor];
    self.frameView.autoresizesSubviews=YES;
    self.frameView.clipsToBounds=YES;
    [self.frameView setOpaque:YES];
    [self.frameView setClearsContextBeforeDrawing:YES];
    self.frameView.autoresizingMask=UIViewAutoresizingNone;
    ...
    //    frameView.clearsContextBeforeDrawing=YES;
